...
const DEFAULT_FILTER_LIST: &str = "||doubleclick.net^\n||googlesyndication.com^";
...
impl PlusApp {
    fn new() -> Result<Self> {
        ...
        let adblock = AdblockEngine::from_filter_list(DEFAULT_FILTER_LIST)?;
        Ok(Self {
            tabs: vec![Tab {
...
fn main() -> Result<()> {
    ...
    let proxy_handle = app.runtime.block_on(start_proxy(
        "127.0.0.1:0",
        DEFAULT_FILTER_LIST.to_string(),
        vpn_socks.clone(),
    ))?;
